...
/**
 * Implementation of the hitman extra entity which chases the player and kills
 * it.
 * 
...
public class Hitman extends Thief {
	 ...
	 * 
	 * @param nextTile
	 *            The tile to start the pathfinding from.
	 * @param visitedTiles
	 *            An array of every tile visited by this pathfinding chain.
	 * @param searchLevel
	 *            The depth of this search (through how many tiles).
	...
	private int shortestPathLength(Tile nextTile, ArrayList<Tile> visitedTiles,
		...
		if (nextTile.hasEntity()
				&& nextTile.getContainedEntity() instanceof Player) {
			lastTile = null;
		...
		currentReturn = shortestPathLength(this.getLevelControl()
				.nextValidTileForHitman(Direction.LEFT, nextTile), tempVisited,
				searchLevel + 1);
		...
		currentReturn = shortestPathLength(this.getLevelControl()
				.nextValidTileForHitman(Direction.RIGHT, nextTile), tempVisited,
				searchLevel + 1);
		...
		currentReturn = shortestPathLength(this.getLevelControl()
				.nextValidTileForHitman(Direction.UP, nextTile), tempVisited,
				searchLevel + 1);
		toReturn = chooseToReturn(toReturn, currentReturn);
		if (toReturn == currentReturn) {
		...
		currentReturn = shortestPathLength(this.getLevelControl()
				.nextValidTileForHitman(Direction.DOWN, nextTile), tempVisited,
				searchLevel + 1);
		toReturn = chooseToReturn(toReturn, currentReturn);
		if (toReturn == currentReturn) {
		...
		if (searchLevel == 0) {
			this.tileToMoveCheat = this.getLevelControl()
					.nextValidTileForHitman(correctDirection, nextTile);
		}
	 ...
	 * 
	 * @param toReturn
	 *            First value to check against the other.
	 * @param currentReturn
	 *            Second value to check against the other.
	...
	public boolean makeMove(Tile tile) {
		...
		for (Direction direction : directions) {
			if (this.getLevelControl().nextValidTileForHitman(direction,
					tile) == tileToMove) {
		...
		if (tileToMove.hasEntity()
				&& tileToMove.getContainedEntity() instanceof Player) {
